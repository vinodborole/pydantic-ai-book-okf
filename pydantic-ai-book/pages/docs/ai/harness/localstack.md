...
`pydantic_ai_harness`re-export:`from pydantic_ai_harness.localstack import LocalStack`The API may change between releases. Where practical, breaking changes ship with a deprecation warning.
...
the capability start and stop a fresh Docker container per run.| Tool | Purpose | 
|---|---|
| `aws_cli` | Run an AWS CLI command against LocalStack. Pass the command withoutthe leading`aws`andwithout`--endpoint-url`— both are injected. Returns labelled stdout/stderr plus an exit code on failure. | 
...
`field(default_factory=(list[str]))`These AWS services are always rejected (denylist).
...
`field(default_factory=(list[str]))`Default timeout in seconds for AWS CLI commands and the health check.
...
`None`If True, mount `/var/run/docker.sock` into the managed container for Docker-backed services like Lambda.
...
`None`Environment variables passed to the managed container, e.g. `{'DEBUG': '1'}`.
...
`field(default_factory=(dict[str, str]))`Path or name of the Docker executable used to manage the container.
...
`AgentDepsT`]**Bases:** `FunctionToolset[AgentDepsT]`
...
`str`The AWS CLI command to run (e.g. `s3 ls`).

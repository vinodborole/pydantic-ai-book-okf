...
[`TemplateStr`](/docs/ai/api/pydantic-ai/agent/) — so `{{field}}` is filled
from `deps`:
...
- **Runs outermost.** The capability wraps[`Instrumentation`](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.Instrumentation) so the resolved variable’s baggage covers the agent run span as well as its children. On recent
...
[`RunContext`](/docs/ai/api/pydantic-ai/tools/#pydantic_ai.tools.RunContext). When `None`, Logfire falls back to its own

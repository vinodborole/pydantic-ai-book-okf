...
To build your own [capability](/docs/ai/capabilities/overview), subclass [ AbstractCapability](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability) and override the methods you need. There are two categories: 

**configuration methods**that are called at agent construction — and re-run at run setup on the replacement instance when

[returns one (see](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.for_run)

`for_run`[Per-run state isolation](#per-run-state-isolation));

[is always called per-run — and](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_wrapper_toolset)
...
`RunContext`For [native tools](/docs/ai/tools-toolsets/native-tools), override [ get_native_tools](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_native_tools) to return a sequence of 
...
Instructions can also use [template strings](/docs/ai/core-concepts/agent-spec#template-strings) ([ TemplateStr('Hello {{name}}')](/docs/ai/api/pydantic-ai/template/#pydantic_ai.template.TemplateStr)) for Handlebars-style templates rendered against the agent’s 

[dependencies](/docs/ai/core-concepts/dependencies). In Python code, a callable with

[is generally preferred for IDE autocomplete.](/docs/ai/api/pydantic-ai/tools/#pydantic_ai.tools.RunContext)

`RunContext`[ get_model_settings](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_model_settings) returns 
...
| `get_native_tools()` | `Sequence[``AgentNativeTool``]` | [Native tools](/docs/ai/tools-toolsets/native-tools)to register (including callables) | 
| `get_wrapper_toolset()` | ``AbstractToolset` | None` | 
| `get_instructions()` | ``AgentInstructions` | None` | 
| `get_model_settings()` | ``AgentModelSettings` | None` | 
| `get_model()` | ``AgentModel` | None` | 
| `resolve_model_id()` | ``Model` | None` | 
...
[,](/docs/ai/api/pydantic-ai/agent/#pydantic_ai.agent.UserPromptNode)

`UserPromptNode`[,](/docs/ai/api/pydantic-ai/agent/#pydantic_ai.agent.ModelRequestNode)

`ModelRequestNode`[). Override this to observe node transitions, add per-step logging, or modify graph progression:](/docs/ai/api/pydantic-ai/agent/#pydantic_ai.agent.CallToolsNode)

`CallToolsNode`You can also use `wrap_node_run` to modify graph progression — for example, limiting the number of model requests per run:
...
[ ModelRequestContext](/docs/ai/api/models/base/#pydantic_ai.models.ModelRequestContext) bundles 

`model`, `messages`, `model_settings`, and `model_request_parameters` into a single object, making the signature future-proof. To swap the model for a given request, set `request_context.model` to a different [instance.](/docs/ai/api/models/base/#pydantic_ai.models.Model)

`Model`To skip the model call entirely and provide a replacement response, raise [ SkipModelRequest(response)](/docs/ai/api/pydantic-ai/exceptions/#pydantic_ai.exceptions.SkipModelRequest) from 

`before_model_request` or `wrap_model_request`.`before_model_request` hooks see the full `request_context.messages` list, including any [message history](/docs/ai/core-concepts/message-history) passed to `agent.run()`, and can modify it.
...
| `on_tool_validate_error` | `(ctx: ``RunContext``, *, call: ``ToolCallPart``, tool_def: ``ToolDefinition``, args: ``, error: ValidationError`RawToolArgs` | `[`ModelRetry`](/docs/ai/api/pydantic-ai/exceptions/#pydantic_ai.exceptions.ModelRetry)`) ->`[`ValidatedToolArgs`](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.ValidatedToolArgs) | 
...
`before_tool_execute` or `wrap_tool_execute`.Tool validation and execution hooks can raise [ ModelRetry](/docs/ai/api/pydantic-ai/exceptions/#pydantic_ai.exceptions.ModelRetry) to request a retry, or 

[to report a failed tool result without retrying. See](/docs/ai/api/pydantic-ai/exceptions/#pydantic_ai.exceptions.ToolFailed)

`ToolFailed`[triggering retries and tool failures](/docs/ai/core-concepts/hooks#triggering-retries-with-modelretry)for the full pattern.
...
| `on_output_validate_error` | `(ctx, *, output_context, output: RawOutput, error: ValidationError | ModelRetry) -> Any` | 
...
`for_run`When `for_run` returns a new instance, the capability’s configuration is re-extracted from that replacement at run setup: [ get_instructions](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_instructions), 

[,](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_toolset)

`get_toolset`[, and](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_native_tools)

`get_native_tools`[are re-invoked on it, and](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_model_settings)

`get_model_settings`[,](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_wrapper_toolset)

`get_wrapper_toolset`[, and all lifecycle hooks always run on it. The exception is model selection:](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_description)

`get_description`[and bootstrap](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.get_model)

`get_model()`[run](/docs/ai/api/pydantic-ai/capabilities/#pydantic_ai.capabilities.AbstractCapability.resolve_model_id)

`resolve_model_id()`*before*

`for_run` on the original instance, and the bootstrap selection is reused unless the replacement actually changes the model contribution — see [Model selection lifecycle and limitations](#model-selection-lifecycle-and-limitations).
...
`ordering` parameter, so you can declare ordering constraints without subclassing:Capabilities don’t have direct access to each other. To share state between capabilities during a run, use a [ contextvars.ContextVar](https://docs.python.org/3/library/contextvars.html#contextvars.ContextVar): one capability sets it (e.g. in 

`wrap_run` or `before_run`), and another reads it from its hooks. The order of capabilities in the `capabilities` list matters — the writer must come before the reader so its `before_*` hook runs first.Test custom capabilities the same way you [test agents](/docs/ai/guides/testing) — using [ TestModel](/docs/ai/api/models/test/#pydantic_ai.models.test.TestModel) or 

[. Create an agent with your capability and assert on the run result, messages, or any observable side effects of your hooks.](/docs/ai/api/models/function/#pydantic_ai.models.function.FunctionModel)

`FunctionModel`A guardrail is a capability that intercepts model requests or responses to enforce safety rules. Here’s one that scans model responses for potential PII and redacts it:

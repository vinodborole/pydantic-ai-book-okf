...
The `XSearch`[capability](/docs/ai/capabilities/overview) gives your agent search over X (Twitter) posts. It’s a [provider-adaptive tool](/docs/ai/capabilities/overview#provider-adaptive-tools) backed by [ XSearchTool](/docs/ai/api/pydantic-ai/native_tools/#pydantic_ai.native_tools.XSearchTool) on the native side — see 

[X Search Tool](/docs/ai/tools-toolsets/native-tools#x-search-tool)for configuration options.

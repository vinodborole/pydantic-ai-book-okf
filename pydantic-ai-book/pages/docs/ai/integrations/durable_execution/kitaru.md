...
resource: https://pydantic.dev/docs/ai/integrations/durable_execution/kitaru
timestamp: '2026-07-07T10:31:51.511921+00:00'
---

# Kitaru

Kitaru is a durable execution layer for AI agents. Its Pydantic AI adapter is provided by the `kitaru` package through `kitaru.adapters.pydantic_ai`, rather than by `pydantic_ai.durable_exec`.
...
You can make a normal `Agent` durable by wrapping it with `KitaruAgent` from `kitaru.adapters.pydantic_ai`. Install Kitaru separately from Pydantic AI:
...
It exposes the usual run methods, including `Agent.run` and `Agent.run_sync`. The original `Agent` can still be used normally outside Kitaru.
...
See the Kitaru Pydantic AI adapter guide for advanced checkpoint configuration.
...
```
For Pydantic AI’s own deferred tool patterns, see deferred tools.

Kitaru supports Pydantic AI streaming with some constraints. For event streaming, prefer the `event_stream_handler` argument on `Agent.run`. When a run uses `event_stream_handler`, Kitaru falls back to a turn checkpoint for that call.

If you use `run_stream()` or `iter()`, wrap the streaming call in an explicit `@kitaru.checkpoint`. This gives Kitaru one durable operation to replay instead of trying to persist each streamed event separately.
...
1. Source page: https://pydantic.dev/docs/ai/integrations/durable_execution/kitaru

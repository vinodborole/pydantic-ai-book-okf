---
type: Web Page
title: V1 → V2 Migration Map | Pydantic Docs
resource: https://pydantic.dev/docs/ai/overview/migration
...
The [Upgrade Guide](/docs/ai/project/changelog/) is the canonical source for *why* each change was made, the behavior changes that come with it, and the recommended upgrade path. This page is the fast path for the one question the guide answers in prose: **what replaced what.**

Most V1 `Agent(...)` arguments that configured behavior moved onto [capabilities](/docs/ai/capabilities/overview/), a single composable primitive that bundles an agent’s tools, [hooks](/docs/ai/core-concepts/hooks/), instructions, and model settings.
...
[`ModelProfile`](/docs/ai/api/pydantic-ai/profiles/#pydantic_ai.profiles.ModelProfile) and its subclasses are now `TypedDict`s rather than dataclasses. Constructing one (`OpenAIModelProfile(field=value)`) is unchanged; reading, mutating, or merging one is not. The full recipe table is in the Upgrade Guide under [`ModelProfile` is now a `TypedDict`](/docs/ai/project/changelog/#modelprofile-is-now-a-typeddict).

| V1 | V2 | 
|---|---|
| `profile.field` | `profile.get('field', <default>)` — defaults are exported from[`pydantic_ai.profiles`](/docs/ai/api/pydantic-ai/profiles/#pydantic_ai.profiles) | 
| `profile.field = value` | `profile['field'] = value` | 
| `dataclasses.replace(profile, field=value)` | `{**profile, 'field': value}` | 
| `profile.update(other)` | [`merge_profile(profile, other)`](/docs/ai/api/pydantic-ai/profiles/#pydantic_ai.profiles.merge_profile) | 
...
- **`system_prompt_role` moves from a model argument into a profile.** If you were already passing`profile=` to the model, merge the setting into that profile rather than replacing it — a second`OpenAIModelProfile(...)` overrides the first wholesale. Profiles are`TypedDict` s in V2, so merging is`{**existing_profile, 'openai_system_prompt_role': 'user'}` or[`merge_profile()`](/docs/ai/api/pydantic-ai/profiles/#pydantic_ai.profiles.merge_profile) .

The per-transport server classes collapsed into a single [`MCPToolset`](/docs/ai/api/pydantic-ai/mcp/#pydantic_ai.mcp.MCPToolset) whose transport is inferred from the arguments you pass. Its defaults differ from the V1 classes’ — notably `max_retries`, `read_timeout`, `init_timeout`, and `elicitation_handler` — so re-read [MCP Client](/docs/ai/mcp/client/) rather than assuming your V1 timeouts carried over.

| V1 | V2 | 
|---|---|
| `MCPServerStdio` ,`MCPServerSSE` ,`MCPServerStreamableHTTP` ,`MCPServerHTTP` | [`pydantic_ai.mcp.MCPToolset`](/docs/ai/api/pydantic-ai/mcp/#pydantic_ai.mcp.MCPToolset) | 
| `FastMCPToolset` (and the`fastmcp` extra) | `MCPToolset` | 
| `load_mcp_servers` | [`pydantic_ai.mcp.load_mcp_toolsets`](/docs/ai/api/pydantic-ai/mcp/#pydantic_ai.mcp.load_mcp_toolsets) | 
| `Agent.run_mcp_servers()` | `async with agent:` | 
...
| `pydantic_ai.native_tools.UrlContextTool` | [`pydantic_ai.native_tools.WebFetchTool`](/docs/ai/api/pydantic-ai/native_tools/#pydantic_ai.native_tools.WebFetchTool) | 
| `builtin=` argument | `native=` | 
| `pydantic_ai.output.DeferredToolCalls` | [`DeferredToolRequests`](/docs/ai/api/pydantic-ai/tools/#pydantic_ai.tools.DeferredToolRequests) | 
| `DeferredToolCalls.tool_calls` | [`DeferredToolRequests.calls`](/docs/ai/api/pydantic-ai/tools/#pydantic_ai.tools.DeferredToolRequests.calls) | 
| `DeferredToolCalls.tool_defs` | Removed — it always returned an empty dict in V1 | 
| `pydantic_ai.toolsets.external.DeferredToolset` | [`ExternalToolset`](/docs/ai/api/pydantic-ai/toolsets/#pydantic_ai.toolsets.ExternalToolset) | 
| `FunctionToolset.tool()` on a context-free callable | [`FunctionToolset.tool_plain()`](/docs/ai/api/pydantic-ai/toolsets/#pydantic_ai.toolsets.FunctionToolset.tool_plain) —`tool()` now raises if the first parameter isn’t a`RunContext` | 
| `pydantic_ai.ext.aci.tool_from_aci` ,`ACIToolset` | Removed; wrap the tool schemas with [`Tool.from_schema`](/docs/ai/api/pydantic-ai/tools/#pydantic_ai.tools.Tool.from_schema) | 
...
| `ModelResponse.builtin_tool_calls` | [`ModelResponse.native_tool_calls`](/docs/ai/api/pydantic-ai/messages/#pydantic_ai.messages.ModelResponse.native_tool_calls) | 
| `ModelResponse.price()` | [`ModelResponse.cost()`](/docs/ai/api/pydantic-ai/messages/#pydantic_ai.messages.ModelResponse.cost) | 
| `Usage` | [`RunUsage`](/docs/ai/api/pydantic-ai/usage/#pydantic_ai.usage.RunUsage) | 
...
| `StreamedRunResult.stream` | [`stream_output`](/docs/ai/api/pydantic-ai/result/#pydantic_ai.result.StreamedRunResult.stream_output) | 
| `StreamedRunResult.stream_structured` | [`stream_response`](/docs/ai/api/pydantic-ai/result/#pydantic_ai.result.StreamedRunResult.stream_response) | 
...
| `StreamedRunResult.validate_structured_output` | [`validate_response_output`](/docs/ai/api/pydantic-ai/result/#pydantic_ai.result.StreamedRunResult.validate_response_output) | 
...
| `evaluation_name` class attribute | [`Evaluator.get_default_evaluation_name()`](/docs/ai/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.Evaluator.get_default_evaluation_name) | 
| `evaluator_version` class attribute | [`Evaluator.get_evaluator_version()`](/docs/ai/api/pydantic_evals/evaluators/#pydantic_evals.evaluators.Evaluator.get_evaluator_version) | 
...
A bare `uv add pydantic-ai` / `pip install pydantic-ai` now installs a slimmer set of extras. `bedrock`, `groq`, `mistral`, `cohere`, `xai`, `huggingface`, `temporal`, `ag-ui`, `ui`, and `spec` are no longer included by default — add the ones you use, e.g. `uv add 'pydantic-ai[bedrock,groq]'`. The `outlines-*`, `vertexai`, `fastmcp`, and `a2a` extras are removed outright. See the [installation guide](/docs/ai/overview/install/) for the full list.

These flip without any symbol changing name, so they can’t be found by grepping for an old name. Each is explained in full in the Upgrade Guide under [changes not covered by deprecation warnings](/docs/ai/project/changelog/#changes-not-covered-by-deprecation-warnings).

- The default `end_strategy` changed from`'early'` to`'graceful'` , so function tools requested alongside a successful output tool now run instead of being skipped. See[Parallel Output Tool Calls](/docs/ai/core-concepts/output/#parallel-output-tool-calls) .
...
- [`capture_run_messages()`](/docs/ai/api/pydantic-ai/agent/#pydantic_ai.agent.capture_run_messages) also captures the partial request/response of an interrupted run, marked`state='interrupted'` .
- A resolved model profile now carries fields from other profile classes, where V1 filtered them out.

# Citations

1. Source page: https://pydantic.dev/docs/ai/overview/migration

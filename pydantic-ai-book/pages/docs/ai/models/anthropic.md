...
...
```
You can customize model behavior using `AnthropicModelSettings`:
...
You can use the unified `service_tier` field or the provider-specific `anthropic_service_tier` field. `anthropic_service_tier` takes precedence over the unified field when both are set, and accepts Anthropic’s native values (`'auto'` or `'standard_only'`).
...
- **Cache the Final Message Block**: Set- `AnthropicModelSettings.anthropic_cache_messages`to- `True`(uses 5m TTL by default) or specify- `'5m'`/- `'1h'`directly
...
Fast mode provides higher output tokens per second and is currently supported on **Claude Opus 4.6**, **Claude Opus 4.7**, and **Claude Opus 4.8**. It is a research preview. Set `anthropic_speed` to `'fast'` to enable it; Pydantic AI automatically adds the required `fast-mode-2026-02-01` beta. On unsupported models, `anthropic_speed='fast'` is ignored with a `UserWarning`. For pricing, rate limits, and the latest list of supported models, see the Anthropic fast mode docs.
...
...
```
Most Anthropic models let you force a tool call via `tool_choice='required'` (or a list of tool names), except while thinking is enabled. **Claude Fable 5** and the **Claude Mythos** models reject a forced tool choice unconditionally — even without thinking — so Pydantic AI marks them with `anthropic_supports_forced_tool_choice=False`.

On a model that doesn’t support forcing:

- An explicit `tool_choice='required'`(or a list of tool names) raises a`UserError`; use`tool_choice='auto'`instead.
- A `required`choice that Pydantic AI resolved on your behalf (e.g. from an output tool) falls back softly to`'auto'`, with the available tools filtered to the requested set so the model can still only pick from them. Filtering the tool definitions invalidates Anthropic’s prompt cache, since the cached prefix includes the tool array.
...
By default, Pydantic AI chooses a compatible Anthropic code execution tool version for the selected model. You can override this with `AnthropicModelSettings.anthropic_code_execution_tool_version` when you need a specific supported Anthropic tool version:

Pydantic AI raises a `UserError` if you explicitly select a tool version that the model does not support.

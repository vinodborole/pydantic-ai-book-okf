...
`AgentSpec` instance and supports additional keyword arguments that supplement or override the spec:Keyword arguments interact with spec fields as follows:
...
| `capabilities` | `list` | [Capabilities](/docs/ai/core-concepts/capabilities)(see[spec syntax](#capability-spec-syntax)) | 
...
`dict[str, Any]`.`AgentSpec.to_file` saves a spec to YAML or JSON and optionally generates a companion JSON Schema file for editor autocompletion:

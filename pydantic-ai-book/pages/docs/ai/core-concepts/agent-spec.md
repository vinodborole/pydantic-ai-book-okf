...
[`Agent.from_spec`](/docs/ai/api/pydantic-ai/agent/#pydantic_ai.agent.Agent.from_spec) accepts a dict or [`AgentSpec`](/docs/ai/api/pydantic-ai/agent/#pydantic_ai.agent.AgentSpec) instance and supports additional keyword arguments that supplement or override the spec:
...
In Python code, [`TemplateStr`](/docs/ai/api/pydantic-ai/template/#pydantic_ai.template.TemplateStr) can be used explicitly, but a callable with [`RunContext`](/docs/ai/api/pydantic-ai/tools/#pydantic_ai.tools.RunContext) is generally preferred for IDE autocomplete and type checking:
...
[`AgentSpec.to_file`](/docs/ai/api/pydantic-ai/agent/#pydantic_ai.agent.AgentSpec.to_file) saves a spec to YAML or JSON and optionally generates a companion JSON Schema file for editor autocompletion:
